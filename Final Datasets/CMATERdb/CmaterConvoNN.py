# -*- coding: utf-8 -*-
"""
Created on Wed Feb 19 13:50:58 2020
...
cmater_data = pd.read_csv('CMATERdb_Final.csv')
#print(ekush_data.isnull().any())

x = cmater_data.iloc[:,1:785]
y = cmater_data.iloc[:,0]
...
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=int(random.uniform(0,100)))
...
x_trainN, x_valid, train_label, valid_label = train_test_split(x_trainN, y_trainN, test_size=0.2, random_state=int(random.uniform(0,100)))
...
bangla_model.add(Conv2D(32, kernel_size=(3, 3),activation='linear',input_shape=(28,28,1), padding='same'))
...
bangla_model.compile(loss=keras.losses.categorical_crossentropy, optimizer=keras.optimizers.Adam(),metrics=['accuracy'])

bangla_model.summary()

bangla_train = bangla_model.fit(x_trainN, train_label, batch_size=batch_size,epochs=epochs,verbose=1,validation_data=(x_valid, valid_label))
...
accuracy = bangla_train.history['acc']
val_accuracy = bangla_train.history['val_acc']
